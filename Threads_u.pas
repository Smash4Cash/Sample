...
type
  ...
  // потоки снятия измерений по COM-порту
  ...
  // поток автокопирования TCP архивов
  TArch = class(TThread)
  TreeMutex2  : THandle;
  private
    DevToFill: TDev;
  ...
  // поток ручного копирования TCP архивов
  ...
  // поток копирования файла по FTP
...
// поток снятия измерений по COM-порту
...
var
  ...
  s : string;
  arData  : array [0..9] of TMeasData; // буфер для чтения архива
  L : integer;
...
begin
  ...
  begin
    ...
    lblLoadD.Caption := 'Загрузка данных...';
    lblLoadG.Caption := 'Загрузка данных...';
    Refresh;
  end;
end;
...
begin
  ...
  begin
    ...
    // данных с таким временем еще не было (или если время неопределено)
    if (LocInd < 0)or(arD[i].DT=0) then
    begin
      if (j > 1) then frmArchive.asgData.AddRow;
      if arD[i].DT <> 0 then
        frmArchive.asgData.AllCells[0,j] := DateToStr(arD[i].DT)+' '+TimeToStr(arD[i].DT) // Время
      else
      ...
      begin
        ...
        if arD[i].DT <> 0 then
          Chan.serArch.AddXY(arD[i].DT,arD[i].Value, '', ErrColor);    // добавляем на график (спец.цвет)
      end
      // нет ошибок
      else begin
        frmArchive.asgData.AllCells[2,j] := '-';                       // выводим '-' в код ошибки
        if arD[i].DT <> 0 then
          Chan.serArch.AddXY(arD[i].DT,arD[i].Value);                  // добавляем на график
      end;
      inc(j);
    end;
...
begin
  ...
  begin
    ...
    MesText := 'Загрузка дерева каталогов завершена.';
  end
  else begin
    MesText := 'Загрузка дерева каталогов остановлена. ' + SysErrorMessage(Err) + '.';
    // прибор (или что-то вместо него) пропинговался, но дальше дело не пошло, т.е. вероятно старая прошивка
    if Err <> TCP_ERROR_BAD_NET_PATH then
    begin
      sVersion := Dev.GetTCPVersion;
      if Length(sVersion) > 0 then
        MesText := MesText + ' ' + 'Внимание! Функция считывания архивов доступна при работе с приборами, имеющими ' +
                                   'версию встроенного ПО не ниже ' + sVersion + '. Скачать последнюю версию прошивки можно ' +
                                   'на сайте www.elemer.ru'
      else
        MesText := MesText + ' ' + 'Внимание! Функция считывания архивов доступна при работе с приборами, имеющими ' +
                                   'актуальную версию встроенного ПО. Скачать последнюю версию прошивки можно ' +
                                   'на сайте www.elemer.ru';
    end;
    if Dev = frmMain.GetSelectedDev(frmMain.TCPTree,0) then
...
begin
  ...
  if not Assigned(Dev) then EXIT;
  MesText := 'Загрузка дерева каталогов...';
  if Dev = frmMain.GetSelectedDev(frmMain.TCPTree,0) then
  begin
...
begin
  ...
  begin
    try
      ...
      on E: EIdReadTimeout do Mess := 'Превышено максимальное время ожидания ответа (' +IntToStr(Trunc(IdFTP.ReadTimeout/1000)) + ' сек)';
